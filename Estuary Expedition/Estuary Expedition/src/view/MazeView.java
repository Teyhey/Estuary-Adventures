...
public class MazeView extends MazeController{
		...
		try {
			...
			if (this.maze.isTutorial()){
				...
				if (this.getTimeLeft() <= 0 || this.maze.player.health <= 0){
					String[] sArr = {"Would you like to try again?"};
					if (this.getTimeLeft() <= 0){
						String s = "Sorry, you ran out of time";
						drawGameOver(g, s, sArr);
					}
					else {
						String s = "Sorry, you got too hurt";
						drawGameOver(g, s, sArr);
					}
				} 
				else {
	...
	/**
	 * This method is responsible for drawing the game-over on Panel.
	 * @param Graphics g, String s, String[] aArr. 
	 * @return void.
	 */
	private void drawGameOver(Graphics g, String s, String[] sArr){
		drawOneLine(g, s);
		drawMoreLines(g, sArr);
